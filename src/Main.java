...
//            }
                ...
                int count = 0;
                Document doc = Jsoup.parse(file, "UTF-8");
                Elements allCategories = doc.getElementsByClass("category");

                for (Element category : allCategories) {
                    if (category.select("h3").first().text().contains("No Force Org Slot"))
                        continue;
                    else {
                        Elements allUnits = category.getElementsByClass("rootselection");
                        //Elements allUnits = allCategories.getElementsByClass("rootselection");
